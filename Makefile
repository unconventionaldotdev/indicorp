...
.PHONY: lint-py
lint-py:
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.py$$' | sed 's|^|indico/|' | xargs -r uv run isort --check-only
	...
	@git -C indico diff --name-only --diff-filter=ACM HEAD | sed 's|^|indico/|' | xargs -r -I{} uv run unbehead --check --path {} > /dev/null
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.py$$' | sed 's|^|indico/|' | xargs -r uv run ruff check --output-format=concise

.PHONY: lint-js
lint-js:
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.\(js\|jsx\|ts\|tsx\)$$' | sed 's|^|indico/|' | xargs -r npx @biomejs/biome ci --files-ignore-unknown=true
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.\(js\|jsx\|ts\|tsx\)$$' | sed 's|^|indico/|' | xargs -r npx eslint
	@cd indico && npx tsc --noEmit
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.\(scss\|css\)$$' | sed 's|^|indico/|' | xargs -r npx stylelint --formatter unix
...
.PHONY: fix-py
fix-py:
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.py$$' | sed 's|^|indico/|' | xargs -r uv run isort
	...
	@git -C indico diff --name-only --diff-filter=ACM HEAD | sed 's|^|indico/|' | xargs -r -I{} uv run unbehead --path {} > /dev/null
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.py$$' | sed 's|^|indico/|' | xargs -r uv run ruff check --fix

.PHONY: fix-js
fix-js:
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.\(js\|jsx\|ts\|tsx\)$$' | sed 's|^|indico/|' | xargs -r npx @biomejs/biome check --write --files-ignore-unknown=true
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.\(js\|jsx\|ts\|tsx\)$$' | sed 's|^|indico/|' | xargs -r npx eslint --fix
	@git -C indico diff --name-only --diff-filter=ACM HEAD | grep '\.\(scss\|css\)$$' | sed 's|^|indico/|' | xargs -r npx stylelint --fix
